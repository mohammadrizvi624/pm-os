...
PM OS uses a **single, lockstep version** across the whole repo. The marketplace
(`.claude-plugin/marketplace.json` → `metadata.version`) and every plugin's
`version` field move together — they are always the same number. The marketplace
is released as one unit, not as independently-versioned plugins.

- **Scheme:** [Semantic Versioning](https://semver.org/) — `MAJOR.MINOR.PATCH`.
  - `MAJOR` — breaking changes to a skill/command contract or the company-context layout.
  - `MINOR` — new plugins, skills, commands, or templates (backward-compatible).
  - `PATCH` — fixes and wording/clarity tweaks with no behavior change.
- **On every release, bump in lockstep:** `metadata.version` in `marketplace.json`
  **and** the `version` in all seven `*/​.claude-plugin/plugin.json` to the same value.
- **Record it:** add an entry to [`CHANGELOG.md`](CHANGELOG.md) (Keep a Changelog format).
- **Tag it (optional):** `git tag v<version> && git push --tags` to mark the release commit.
